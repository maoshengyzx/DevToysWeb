  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 border-b border-border px-6 py-3 flex-wrap">
        <span className="text-sm text-muted-foreground">{t("tool.imageCompress.format")}</span>
        ...
        {format !== "png" && (
          <>
            ...
            <span className="text-sm text-muted-foreground">{t("tool.imageCompress.quality")}: {quality}%</span>
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="space-y-6 max-w-4xl mx-auto">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-foreground">{t("tool.imageCompress.uploadImage")}</label>
            <label className="flex flex-col items-center gap-2 rounded-md border border-dashed border-border px-4 py-8 cursor-pointer hover:border-primary transition-colors duration-150">
              ...
              <span className="text-sm text-muted-foreground">{t("tool.imageCompress.uploadHint")}</span>
              <input type="file" accept="image/*" className="hidden" onChange={handleUpload} />
            </label>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-sm text-muted-foreground">{t("tool.imageCompress.original")}</label>
              {sourceInfo.size > 0 && (
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm text-muted-foreground">{t("tool.imageCompress.result")}</label>
              {resultInfo.size > 0 && (
                <div className="text-xs text-muted-foreground space-y-0.5">
                  <p>{resultInfo.width} × {resultInfo.height} px</p>
                  <p>{formatSize(resultInfo.size)}</p>
                  {savings > 0 && <p className="text-green-500">{t("tool.imageCompress.saved").replace("{savings}", String(savings))}</p>}
                </div>
              )}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium text-foreground">{t("tool.imageCompress.maxWidth")}</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium text-foreground">{t("tool.imageCompress.maxHeight")}</label>
          ...
          {sourceUrl && (
            <div className="flex gap-2">
              <Button className="cursor-pointer" onClick={handleReprocess}>{t("tool.imageCompress.reprocess")}</Button>
              {resultUrl && (
          ...
          {sourceUrl && (
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-foreground">{t("tool.imageCompress.original")}</label>
                <img src={sourceUrl} alt="Original" className="max-h-[300px] rounded-md border border-border object-contain" />
              </div>
              {resultUrl && (
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-foreground">{t("tool.imageCompress.result")}</label>
